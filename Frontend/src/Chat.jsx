...
export default function Chat() {
  ...
  // Persist whenever the message list changes.
  useEffect(() => {
    try {
  ...
  // Keep the latest message in view (only while the panel is open).
  useEffect(() => {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end gap-3">
      ...
      {open && (
        <section className="flex h-[28rem] w-[22rem] max-w-[calc(100vw-2rem)] flex-col rounded-lg border border-gray-200 bg-white shadow-xl">
          ...
          {/* Message history (flex-1 fills the fixed-height panel) */}
          <div className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
            {messages.length === 0 && (
              <p className="text-sm text-gray-400">
                Try “Which venue has the most active sessions?” or “What's the
